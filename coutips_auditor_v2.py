...
def _similaridade_nome(a: str, b: str) -> float:
    """Similaridade com múltiplas estratégias — PRECAUÇÕES 3 e 4 (01/07):
    o matching por nome falhava silenciosamente em casos comuns de produção:
    - nomes com acento (São Paulo vs Sao Paulo)
    - abreviações (Man City vs Manchester City)
    - ordem invertida (Brazil U20 vs U20 Brazil)
    - sufixos de categoria (Bayern Munich vs Bayern München)
    - siglas (PSG vs Paris Saint-Germain)
    Agora tenta 5 estratégias e retorna a maior similaridade encontrada.
    """
    def _norm(t: str) -> str:
        return _normalizar_nome_time(t)

    def _seq(x: str, y: str) -> float:
        from difflib import SequenceMatcher
        return SequenceMatcher(None, x, y).ratio()

    def _palavras(x: str, y: str) -> float:
        sx, sy = set(x.split()), set(y.split())
        if not sx or not sy:
            return 0.0
        return len(sx & sy) / max(len(sx), len(sy))
    ...
    scores = []
    # Estratégia 1: palavras em comum (original)
    scores.append(_palavras(na, nb))
    # Estratégia 2: sequência de caracteres (captura abreviações parciais)
    scores.append(_seq(na, nb))
    # Estratégia 3: ordem invertida (ex: "U20 Brazil" vs "Brazil U20")
    na_inv = " ".join(reversed(na.split()))
    scores.append(_palavras(na_inv, nb))
    scores.append(_seq(na_inv, nb))
    # Estratégia 4: primeiro token vs primeiro token (siglas vs nome completo)
    tok_a, tok_b = na.split()[0] if na.split() else "", nb.split()[0] if nb.split() else ""
    if tok_a and tok_b and tok_a == tok_b:
        scores.append(0.75)  # mesmo primeiro token = provável mesmo time
    # Estratégia 5: um nome contém o outro inteiro
...
class CornerProProvider(BaseProvider):
    ...
    def _pronto_para_buscar(self, match: Dict[str, Any]) -> bool:
        ...
        que a decisão foi registrada na fila.

        PRECAUÇÃO 10 (01/07): margem adaptativa para jogos de copa/prorrogação.
        Alertas de HT (minuto <= 45) podem ser de jogos que ainda vão ter
        prorrogação — nesses casos, 25 minutos de margem pode não ser suficiente.
        Usa margem maior (45 min) para alertas de 1T, e a margem padrão para
        alertas de 2T. O auditor nunca perde resultado — se o jogo ainda não
        terminou (status != 3), a fila tenta de novo no próximo ciclo de 5min.
        """
        try:
            ...
            # Margem adaptativa: HT usa 45min, FT usa o padrão configurado
            if minuto <= 45:
                margem = max(config.cornerpro_margem_minutos, 45)
            else:
                margem = config.cornerpro_margem_minutos

            minutos_espera = max(0, 90 - minuto) + margem
        ...
        def _par(valor) -> Tuple[Optional[int], Optional[int]]:
            ...
            if isinstance(valor, dict):
                try:
                    ...
                    a = valor.get("away") or valor.get("awayScore") or valor.get("a")
