...
CORTE_GOL = int(os.getenv("CORTE_GOL", "80"))
CORTE_CANTO = 999
...
CONFIRMACAO_SCORE_MINIMO = int(os.getenv("CONFIRMACAO_SCORE_MINIMO", "80"))
...
def classificar_liga(competicao):
    ...
    ligas_perigosas = [
        ...
        "greece", "colombia", "argentina primera c", "iraq",
        "bolivia", "peru", "ecuador", "chile", "mongolia", "jordan",
    ]
...
def ajuste_gol_recente_contextual(metricas, estrategia):
    ...
    # REGRA NOVA:
    # Confirmação com gol recente fica mais rígida.
    # Se o dominante/time que pressionava marcou nos últimos 5 min,
    # bloqueia/rebaixa forte: pressão já foi premiada.
    # Exceção: se quem marcou foi o time pressionado/zebra contra o fluxo,
    # o jogo pode abrir e a confirmação continua válida.
    if eh_confirmacao(estrategia) and minutos <= 5:
        if lado != "DESCONHECIDO":
            ...
            if lado == dom:
                return -35, "TRAVA_CONFIRMACAO_PRESSAO_PREMIADA_5M"
...
def score_delta_confirmacao(metricas, estrategia, chave_jogo):
    ...
    for campo, peso in [
        ...
        if sum(metricas.get(campo, (0, 0))) > sum(ant.get(campo, (0, 0))):
            score += peso

    if sum(metricas.get("ultimos5", (0, 0))) < max(1, sum(ant.get("ultimos5", (0, 0))) - 2):
        score -= 4

    if sum(metricas.get("ultimos10", (0, 0))) < max(1, sum(ant.get("ultimos10", (0, 0))) - 3):
        score -= 4
...
def score_gol(metricas, estrategia, chave_jogo):
    ...
    bonus, padrao = score_padrao_alfa(metricas, estrategia)
    score += bonus
    ...
    score += int(favorito_score(metricas) * (0.65 if eh_confirmacao(estrategia) else 0.75))
    score += 4 if eh_confirmacao(estrategia) else 5
...
def confirmacao_melhorou_forte(alerta):
    ...
    ] and score_gol >= CORTE_GOL:
        return True

    # Confirmação normal só vai ao principal se realmente melhorou.
    if delta_ctx == "CONFIRMACAO_MELHOROU" and score_gol >= CONFIRMACAO_SCORE_MINIMO:
        return True
...
async def processar_evento(event, origem="nova"):
    ...
    try:
        ...
        if sg < CORTE_GOL:
            log(f"⛔ BLOQUEADO POR SCORE | {estrategia} | Gol={sg}% < {CORTE_GOL}% | {jogo}")
            return
...
async def main():
    ...
    log(f"🎯 Corte gol: {CORTE_GOL}% | Cantos desativados temporariamente")
    ...
    log("🧠 Score ALFA: somente gols, confirmação por melhora real, trava de gol recente e HT elite com padrão Cruzeiro.")
